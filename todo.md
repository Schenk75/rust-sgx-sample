...
- 完善wasmi-ra代碼：
  ...
  - step4：實現Ocall函數 `ocall_load_wast`，根據客戶端輸入的文件名，讀取(並解封)在Enclave外的文件內的wasm模塊的json字符串，加載進Enclave
...
1. 驗證代碼的完整性（參考tr-mpc）
2. 對於沒有指定CA籤發的證書的用戶，即非Enclave擁有者，如何建立TLS通信（暫時考慮使用不同的CA籤發證書）（這一部分不一定要實現）
